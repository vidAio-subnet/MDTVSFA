...
#         if model == 'AlexNet':
#             print("use AlexNet")
#             self.features = nn.Sequential(*list(get_model('alexnet', weights="DEFAULT").children())[:-2])
#         elif model == 'ResNet-152':
#             print("use ResNet-152")
#             self.features = nn.Sequential(*list(get_model('resnet152', weights="DEFAULT").children())[:-2])
#         elif model == 'ResNeXt-101-32x8d':
#             print("use ResNetXt-101-32x8d")
#             self.features = nn.Sequential(*list(get_model('resnext101_32x8d', weights="DEFAULT").children())[:-2])
#         elif model == 'Wide ResNet-101-2':
#             print("use Wide ResNet-101-2")
#             self.features = nn.Sequential(*list(get_model('wide_resnet101_2', weights="DEFAULT").children())[:-2])
#         else:
#             print("use default ResNet-50")
#             self.features = nn.Sequential(*list(get_model('resnet50', weights="DEFAULT").children())[:-2])
...
#             while frame_end < video_length:
#                 batch = video_data[frame_start:frame_end].to(device, non_blocking=True)
#                 features_mean, features_std = extractor(batch)
#                 output1 = torch.cat((output1, features_mean), dim=0)
#                 output2 = torch.cat((output2, features_std), dim=0)
#                 frame_end += frame_batch_size
#                 frame_start += frame_batch_size
#                 del batch, features_mean, features_std
#                 torch.cuda.empty_cache()
#                 # Debug memory usage
...
#             last_batch = video_data[frame_start:video_length].to(device, non_blocking=True)
#             features_mean, features_std = extractor(last_batch)
#             output1 = torch.cat((output1, features_mean), dim=0)
#             output2 = torch.cat((output2, features_std), dim=0)
#             output = torch.cat((output1, output2), dim=1).squeeze()
#             del last_batch, features_mean, features_std
#             torch.cuda.empty_cache()
#             # Debug memory usage
#             print(f"Final allocated memory: {torch.cuda.memory_allocated() / 1e9:.2f} GB")
#             print(f"Final reserved memory: {torch.cuda.memory_reserved() / 1e9:.2f} GB")
...
#     parser.add_argument('--model', default='ResNet-50', type=str, help='which pre-trained model used (default: ResNet-50)')
#     parser.add_argument('--frame_batch_size', type=int, default=8, help='frame batch size for feature extraction (default: 8)')
#     parser.add_argument('--disable_gpu', action='store_true', help='flag whether to disable GPU')
#     parser.add_argument("--ith", type=int, default=0, help='start frame id')
#     args = parser.parse_args()
...
#         start = time.time()
#         current_data = dataset[i]
#         print(f'Video {i}: length {current_data["video"].shape[0]}')
#         if max_len < current_data['video'].shape[0]:
#             max_len = current_data['video'].shape[0]
#         features = get_features(current_data['video'], args.frame_batch_size, args.model, device)
#         np.save(f'{features_dir}{i}_{args.model}_last_conv.npy', features.to('cpu').numpy())
#         np.save(f'{features_dir}{i}_score.npy', current_data['score'])
#         end = time.time()
